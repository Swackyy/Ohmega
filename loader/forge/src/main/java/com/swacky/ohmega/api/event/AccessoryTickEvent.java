...
public sealed interface AccessoryTickEvent {
    record Pre(Player player, ItemStack stack) implements AccessoryTickEvent, RecordEvent, Cancellable {
        public static final CancellableEventBus<@NonNull Pre> BUS = CancellableEventBus.create(Pre.class);
    }

    record Post(Player player, ItemStack stack) implements AccessoryTickEvent, RecordEvent, Cancellable {
