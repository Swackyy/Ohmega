...
public final class AccessoryMenus {
     ...
     */
    public static List<AccessorySlot> createSlots(@NonNull AbstractContainerMenu menu, @NonNull Player owner, AccessorySlot.@NonNull Factory slotFactory, @Nullable Consumer<AccessorySlot> consumer) {
    ...
    public static void onConstruct(@NonNull AbstractContainerMenu menu, @NonNull Player owner) {
        AccessoryMenuExtension extension = attachExtension(menu, owner, assertImplementation(menu));

        extension.setSlots(createSlots(menu, owner, extension::createSlot, menu::addSlot));
    }

    /**
     * Removes all current slots and calls {@link AccessoryMenuExtension#addSlots(AccessoryMenuExtension.SlotAdder, AccessoryData)} again.
