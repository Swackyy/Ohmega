 ...
 */
// todo: complete scrolling overflow functionality for GUI
public final class DefaultScreenExtension extends AccessoryScreenExtension implements IEntityRenderingExtension {
    ...
    public void initExtension(WidgetAdder adder) {
        ...
        if (size > 0) {
            ...
            renderColumns = (int) Math.min(Math.ceil((double) size / renderSlots), maxColumns);
            slotsAvailable = Math.min(renderColumns * renderSlots, size);
            mostSlotsPerColumn = Math.min(renderSlots, size);
            lastColumnSlots = slotsAvailable % mostSlotsPerColumn == 0 ? mostSlotsPerColumn : slotsAvailable % mostSlotsPerColumn;
        } else {
            renderColumns = 0;
            slotsAvailable = 0;
            mostSlotsPerColumn = 0;
            lastColumnSlots = 0;
        }
