...
public class AccessoryInventoryScreen extends EffectRenderingInventoryScreen<AccessoryInventoryMenu> {
    ...
    protected void renderTooltip(@NotNull GuiGraphics gui, int mx, int my) {
        if(this.menu.getCarried().isEmpty() && this.hoveredSlot != null && !this.hoveredSlot.hasItem() && this.minecraft != null) {
            // Normal
            for(int i = 0; i < 3; i++) {
                ScreenHelper.displayIfBetween(leftPos + 182, leftPos + 199, topPos + 24 + i * 18, topPos + 41 + i * 18, mx, my, () -> gui.renderTooltip(this.minecraft.font, MutableComponent.create(AccessoryType.NORMAL.getTranslation()), mx, my));
            }


            // Utility
            for(int i = 0; i < 2; i++) {
                ScreenHelper.displayIfBetween(leftPos + 182, leftPos + 199, topPos + 78 + i * 18, topPos + 95 + i * 18, mx, my, () -> gui.renderTooltip(this.minecraft.font, MutableComponent.create(AccessoryType.UTILITY.getTranslation()), mx, my));
            }

            // Special
            ScreenHelper.displayIfBetween(leftPos + 182, leftPos + 199, topPos + 114, topPos + 131, mx, my, () -> gui.renderTooltip(this.minecraft.font, MutableComponent.create(AccessoryType.SPECIAL.getTranslation()), mx, my));
